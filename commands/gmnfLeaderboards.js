module.exports = {
	...
	async execute(args, message, client, databases) {
        ...
        gmnfs = ['Corrupted', 'Festering Core', 'Lake of Shadows', 'Garden World',
        ...
        for (let i = 0; i < gmnfs.length; i++) {
            ...
            for (let x = 0; x < allEach.length; x++) {
                comps[allEach[x]['discordName']] = allEach[x]['clears'];
                let ti = allEach[x]['bestTime'].split(" ");
                let ov = 0;
                for (let y = 0; y < ti.length; y++) {
                    if (ti[y] == 'N/A') {
                        ov = 10000000000000000000000000000000000;
                    }
                    else {
                        if (ti[y].includes('h')) {
                            ov = ov + (parseInt(ti[y].slice(0, -1)) * 3600)
                        }
                        else if (ti[y].includes('m')) {
                            ov = ov + (parseInt(ti[y].slice(0, -1)) * 60)
                        }
                        else if (ti[y].includes('s')) {
                            ov = ov + parseInt(ti[y].slice(0, -1))
                        }
                    }
                }
                times[allEach[x]['discordName']] = {
                    'bestTime': allEach[x]['bestTime'],
                    'seconds': ov
                }
            }
            ...
            items2 = Object.keys(times).map(function(key) {
                return [key, times[key]['seconds'], times[key]['bestTime']];
            });
